...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        images = data['images']  # [B, N, 3/4]:  batch?, number?, channel
    ...
    def test_step(self, data,
        ...
        pred_depth_0 = None
        if render_def:
            pred_depth_0 = outputs['depth_0'].reshape(-1, H, W)

        return pred_rgb, pred_depth, pred_depth_0
    ...
    def save_point_cloud(self, save_path=None, resolution=256, threshold=10):

        if save_path is None:
            save_path = os.path.join(self.workspace, 'points', f'{self.name}_{self.epoch}.ply')
        ...
        # points = extract_point_cloud(self.model.aabb_infer[:3], self.model.aabb_infer[3:], resolution=resolution, query_func=query_func)
        points, _ = extract_geometry(self.model.aabb_infer[:3], self.model.aabb_infer[3:], resolution=resolution,
                                     threshold=threshold, query_func=query_func)

        # print(self.model.aabb_infer[:3], self.model.aabb_infer[3:])

        write_to_ply(points, save_path)
        # print(points)
    ...
    def test_gui(self, pose, intrinsics, W, H, bg_color=None, spp=1, downscale=1,
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=self.fp16):
                ...
                preds, preds_depth, preds_depth_0 = self.test_step(data,
                                                        bg_color=bg_color,
                                                        perturb=False if spp == 1 else spp,
                                                        render_def=render_def,
                                                        gui_sim=gui_sim,
                                                        solver=solver,
                                                        frame=frame,
                                                        paused=paused
        ...
        if downscale != 1:
            ...
            if render_def:
                preds_depth_0 = F.interpolate(preds_depth_0.unsqueeze(1), size=(H, W), mode='nearest').squeeze(1)
        ...
        pred_depth = preds_depth[0].detach().cpu().numpy()
        if render_def:
            pred_depth_0 = preds_depth_0[0].detach().cpu().numpy()

        outputs = {
            'image': pred,
            'depth': pred_depth,
            'depth_0': pred_depth_0,
            'rays_o': rays['rays_o'],
            'rays_d': rays['rays_d'],
        }
