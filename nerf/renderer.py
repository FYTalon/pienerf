...
def plot_pointcloud(pc, color=None):
    ...
    scene = trimesh.Scene([pc, axes, sphere])
    scene.set_camera(angles=[0,70,0], distance=10, center=[0, 0, 0])
    scene.show()

def get_cell_for_point(pos, gs):
    return np.array(np.floor(pos / gs).astype(int))
...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-2, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.training:
            ...
            while step < max_steps:
                ...
                xyzs, dirs, deltas = raymarching.march_rays(
                    n_alive, n_step, rays_alive,
                    rays_t, rays_o, rays_d,
                    self.bound, self.density_bitfield, self.cascade, self.grid_size,
                    nears, fars, 128, perturb if step == 0 else False,
                    dt_gamma, max_steps)
    ...
    def render_deformed(self, rays_o, rays_d, staged=False, max_ray_batch=4096, **kwargs):
        ...
        results = _rund(rays_o, rays_d, **kwargs)

        return results


    def rund(self, rays_o, rays_d, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # bg_color: [3] in range [0, 1]
        # return: image: [B, N, 3], depth: [B, N]
        ...
        print(f'rund(): nears = {nears.min().item()} ~ {nears.max().item()}, fars = {fars.min().item()} ~ {fars.max().item()}')

        z_vals = torch.linspace(0.0, 1.0, num_steps, device=device).unsqueeze(0)  # [1, T]
        z_vals = z_vals.expand((N, num_steps))  # [N, T] = [4096, 512]
        z_vals = nears + (fars - nears) * z_vals  # [N, T], in [nears, fars]
        ...
        # # debug----------------------------------------------------------------------
        # xyzs_cpu = xyzs.reshape(-1, 3).detach().cpu().numpy()
        # xyzs_cpu_ymax = xyzs_cpu[:, 1].max()
        # xyzs_cpu_ymin = xyzs_cpu[:, 1].min()
        # colors = np.zeros((xyzs_cpu.shape[0], 4), dtype=np.uint8)
        # for i, point in enumerate(xyzs_cpu):
        #     ratio = (point[1] - xyzs_cpu_ymin) / (xyzs_cpu_ymax - xyzs_cpu_ymin)
        #     red = int((1 - ratio) * 255)
        #     green = int(ratio * 255)
        #     colors[i] = [red, green, 0, 50]
        # colors = np.full((xyzs_cpu.shape[0], 4), [0, 255, 0, 50], dtype=np.uint8)
        # plot_pointcloud(xyzs_cpu, color=colors)
        # # debug----------------------------------------------------------------------
        ...
        ###################
        # added: handled query-failed points: set color to bg_color
        zero_indices = (xyzs.reshape(-1, 3) == torch.tensor([np.nan, np.nan, np.nan], device=xyzs.device)).all(-1)
        density_outputs['sigma'][zero_indices] = 0
        ...
        mask = weights > 1e-4  # hard coded
        # rgbs = self.color(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), mask=mask.reshape(-1), **density_outputs)
        rgbs = self.color(xyzs.reshape(-1, 3), xyzs.reshape(-1, 3), mask=mask.reshape(-1), **density_outputs)
        rgbs = rgbs.view(N, -1, 3)  # [N, T+t, 3]
    ...
    def rund_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, max_steps=1024,
        ...
        num_seek_IP = kwargs.get('num_seek_IP')  # int, ==16
